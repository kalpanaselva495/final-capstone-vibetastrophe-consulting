...
processed_file_path = PROJECT_ROOT / 'data' / 'processed' / 'processed_city_traffic_accidents.csv'
...
cleansed_city_df = clean_data(test_df)
engineered_city_df = engineer_features(cleansed_city_df)

test_file_path = PROJECT_ROOT / 'test_data' / 'test_city_traffic_accidents.csv'
engineered_city_df.to_csv(test_file_path, index=False)
print(f'Test file created at: {test_file_path} with {num_rows} random rows from the original processed file.')
